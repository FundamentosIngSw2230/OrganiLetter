...
public class Planificador
{
    ...
    //Dados una direccion y un arreglo de cartas, la funcion encuentra la carta mas cercana a dicha direccion
    // a = cartasEntregarNormal, b = cartasRecogerNormal, c = cartasEntregarExpres, d = cartasRecogerExpres
    public Carta cartaMasCercana(int calle, int carrera, char tipoDeArreglo) {
        Carta cartaMasCercana = new Carta(alcanceMaxCalle*2 + 1, alcanceMaxCarrera*2 + 1, alcanceMaxCalle*2 + 1, alcanceMaxCarrera*2 + 1);
        if (tipoDeArreglo == 'a') {
    ...
    public ArrayList<Carta> calcularRuta(char tipoDeArreglo){
        ...
        if (tipoDeArreglo == 'a' && cartasEntregarNormal.size() != 0) {
            ...
            for (int i = 0; (i < maxCartasAentregar) && (cartasEntregarNormal.size() !=0 ); i++) {
                cartaMasCercana = cartaMasCercana(cartaMasCercana.getCalleEntrega(), cartaMasCercana.getCarreraEntrega(), tipoDeArreglo);
